...
export default function CardHorizontal({post}: any) {
    ...
    const featuredImageURL = featuredImage?.node?.mediaDetails?.sizes[0]?.sourceUrl ?? [];
    ...
    return (
        <div className="flex gap-8">
            ...
            <div className="flex flex-col gap-4">
                <h3 className="text-2xl	font-medium	text-title hover:text-titleHover">
                    <a href={post.link}>
                        {post.title}
                    </a>
                </h3>
                ...
                <div className="flex gap-4 justify-start items-center">
                    <a href={post.categories.nodes[0].link} className="py-2 px-4 bg-button text-link opacity-100 hover:opacity-70 transition-all duration-500 linear">
